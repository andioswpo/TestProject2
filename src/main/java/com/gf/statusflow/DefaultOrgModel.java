...
public class DefaultOrgModel implements IOrgModel{
	...
	public FunctionInfo initFunc()
	{
		...
		if(workitem == null)
		{
			...
			workitem.setFullName("/功能模块/财务管理/代办工作");
			this.saveFunc(workitem);
		}
		String finishitemId = "func000000002110";
		FunctionInfo finishitem = getFuncById(finishitemId);
		if(finishitem == null)
		{
			finishitem = new FunctionInfo();
			finishitem.setId(finishitemId);
			finishitem.setName("办结工作");
			finishitem.setIcon("pic_21");
			finishitem.setParentId(fi.getId());
			finishitem.setPriority(1);
			finishitem.setUrl("/finishitem.action");
			finishitem.setPath("/func000000000000/func000000002000/func000000002200");
			finishitem.setFullName("/功能模块/财务管理/办结工作");
			this.saveFunc(finishitem);
		}
